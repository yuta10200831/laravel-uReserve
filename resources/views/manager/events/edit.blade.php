    ...
    <div class="max-w-2xl py-4 mx-auto">
     ...
     <form method="POST" action="{{ route('events.update', ['event' => $event->id ]) }}">
      ...
      <div>
       ...
       <x-input id="event_name" class="block mt-1 w-full" type="text" name="event_name" value="{{ $event->name }}"
        required autofocus />
      </div>
      <div class="mt-4">
       <x-label for="information" value="イベント詳細" />
       <x-textarea row="3" id="information" name='information' class="block mt-1 w-full">{{ $event->information }}
       </x-textarea>
      </div>
      ...
      <div class="md:flex justify-between items-end mt-4">
       ...
       <div class="flex space-x-4 justify-around">
        ...
        <input type="radio" name="is_visible" value="1" @if($event->is_visible === 2 ){ checked } @endif />非表示
       </div>
